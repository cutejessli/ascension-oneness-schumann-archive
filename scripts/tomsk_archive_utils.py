...
# Tomsk leaves an empty future section to the right of the most recent chart
# data. A small SOS legend can sit beyond that gap, so edge-only trimming does
# not remove it. Detect the first sustained empty run after the populated chart
# instead. Quiet chart data remains safe because its grid/trace content is not
# an all-but-empty column run.
TRAILING_FUTURE_GAP_START_RATIO = float(os.environ.get("TRAILING_FUTURE_GAP_START_RATIO", "0.45"))
TRAILING_FUTURE_GAP_MIN_WIDTH_PX = int(os.environ.get("TRAILING_FUTURE_GAP_MIN_WIDTH_PX", "32"))
TRAILING_FUTURE_GAP_NON_DARK_RATIO = float(os.environ.get("TRAILING_FUTURE_GAP_NON_DARK_RATIO", "0.01"))
...
def trim_trailing_future_gap(image: Image.Image) -> tuple[Image.Image, dict[str, Any]]:
    """Remove a large empty future segment before a detached Tomsk legend.

    This deliberately looks from left to right rather than from the outer
    right edge. The raw Tomsk image can contain a small non-empty SOS/color
    legend after an otherwise empty future segment, which defeats ordinary
    right-edge trimming. A gap must be both late in the image and sustained
    before it is considered disposable.
    """

    gray = image.convert("L")
    width, height = gray.size
    search_start = max(1, min(width - 1, int(width * TRAILING_FUTURE_GAP_START_RATIO)))
    minimum_gap = max(8, TRAILING_FUTURE_GAP_MIN_WIDTH_PX)
    gap_start: int | None = None

    for x in range(search_start, width):
        if column_non_dark_ratio(gray, x) <= TRAILING_FUTURE_GAP_NON_DARK_RATIO:
            if gap_start is None:
                gap_start = x
            if x - gap_start + 1 >= minimum_gap:
                trimmed = image.crop((0, 0, gap_start, height))
                return trimmed, {
                    "detected": True,
                    "start_px": gap_start,
                    "trimmed_px": width - gap_start,
                    "minimum_gap_width_px": minimum_gap,
                    "search_start_px": search_start,
                    "non_dark_ratio_threshold": TRAILING_FUTURE_GAP_NON_DARK_RATIO,
                    "original_width": width,
                    "trimmed_width": trimmed.width,
                }
        else:
...
def crop_processed_chart(image: Image.Image) -> tuple[Image.Image, dict[str, Any]]:
    ...
    chart_without_future_gap, future_gap_info = trim_trailing_future_gap(chart_image)
    trimmed_chart, trim_info = trim_empty_edge_columns(chart_without_future_gap)
    return trimmed_chart, {
        "mode": "chart_crop_plus_future_gap_and_conservative_edge_trim",
        "chart_crop": {
        ...
        "trailing_future_gap": future_gap_info,
        "edge_trim": trim_info,
